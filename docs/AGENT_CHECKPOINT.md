...
- `2026-05-25` для SEO создана отдельная ветка `seo-hardening` и отдельный `worktree` `/home/max/MADCORE RF SEO`;
- в этой SEO-ветке уже подготовлены safe-правки без изменения визуала:
  - `canonical` для индексируемых страниц;
  - `JSON-LD` со схемами `WebSite`, `Organization`, `Product`;
  - поддержка `GOOGLE_SITE_VERIFICATION` через env;
...
- если продолжается SEO-ветка, следующий шаг уже не кодовый, а операционный:
  - получить токен Search Console и подставить `GOOGLE_SITE_VERIFICATION`;
