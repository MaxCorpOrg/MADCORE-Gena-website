...
- `2026-05-25` для безопасной SEO-работы создана отдельная ветка `seo-hardening` и отдельный `worktree` `/home/max/MADCORE RF SEO`;
- в этой SEO-ветке уже подготовлены:
...
- если продолжаем SEO-ветку, следующим практическим действием будет получение токена Search Console и заполнение `GOOGLE_SITE_VERIFICATION`.
